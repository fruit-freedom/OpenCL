...
int main() {
	...
	// ***********************************************
	Mat image;
	image = imread("D:\\tests\\test-opencl\\image.jpg", IMREAD_COLOR);   // Read the file
	if (!image.data) {
	...
	// 			buffer[4 * (j * image.cols + k)] = image.data[chans * (j * image.cols + k)];
	// 		}
	// 	}
	// }
	...
	// size_t source_size = strlen(src_img_test_1);
	auto program_sources = ReadFile("D:\\tests\\test-opencl\\blur.h");
	size_t source_size = program_sources.length();
	const char* source_data = program_sources.data();
	// cl_program program = clCreateProgramWithSource(context, 1, &src_img_test_1, (const size_t*)&source_size, &errcode_ret);
	cl_program program = clCreateProgramWithSource(context, 1, &source_data, (const size_t*)&source_size, &errcode_ret);
	if (errcode_ret != CL_SUCCESS) {
	...
	// ***********************************************
	errcode_ret = CL_SUCCESS;
	cl_kernel kernel = clCreateKernel(program, "blur_image", &errcode_ret);
	if (errcode_ret != CL_SUCCESS)
	{
	...
	std::cout << "Kernel duration: " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() << std::endl;
	// ***********************************************
